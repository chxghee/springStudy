...
<head>
  ...
  $(document).ready(function(){
	 ...
	 // console.log('${user.member}');	 // 개발자 도구에서 출력값 확인 가능 
	 // console.log('${user.username}');
	 // console.log('${user.member.username}'); 위와 같은 값 
	 console.log('${auth[0]}');	// 배열로 값을 빼오기 가능
	  ...
	  $("button").on("click", function(e){		// 버튼에서 
		  ...
		  if (oper == 'register') {
			  ...
			  location.href = "${cpath}/board/list";	// 리스트 요청 보내기
		  }
		  else if (oper == 'remove') {		// 삭제 눌리면 서버로 인덱스 값을  넘김 -> 처리 후 list 로 back 
			  var idx = regForm.find("#idx").val();
			  location.href= "${cpath}/board/remove?idx="+idx;	// 요청 get 방식으러 넘기기 
		  }
